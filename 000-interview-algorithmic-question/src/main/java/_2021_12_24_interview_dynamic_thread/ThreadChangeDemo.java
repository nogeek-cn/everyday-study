package _2021_12_24_interview_dynamic_thread;
...
public class ThreadChangeDemo {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(ThreadChangeDemo.class);
    ...
    private static void threadPoolStatus(ThreadPoolExecutor executor, String name) {
        ResizableCapacityLinkedBlockingQueue queue = (ResizableCapacityLinkedBlockingQueue) executor.getQueue();
        LOGGER.info(String.format("[threadName:%s][taskName:%s][核心线程数:%s][活动线程数:%s][最大线程数:%s]" +
                        "[线程活跃度:%s][任务完成数:%s][队列大小:%s]" +
                        "[当前排队线程数:%s][队列剩余大小:%s][队列实用度:%s]",
                Thread.currentThread().getName(),
                name, executor.getCorePoolSize(),
     ...
     * @author darian1996
     */
    public static class ResizableCapacityLinkedBlockingQueue<E> extends AbstractQueue<E>
